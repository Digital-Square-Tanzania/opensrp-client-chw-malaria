...
import static org.smartregister.chw.malaria.util.JsonFormUtils.cleanString;
...
public class NCUtils {
    ...
    public static SimpleDateFormat getSourceDateFormat() {
        return new SimpleDateFormat(MalariaLibrary.getInstance().getSourceDateFormat(), Locale.getDefault());
    }

    public static SimpleDateFormat getSaveDateFormat() {
        return new SimpleDateFormat(MalariaLibrary.getInstance().getSaveDateFormat(), Locale.getDefault());
    }
    ...
    public static Visit eventToVisit(Event event, String visitID) throws JSONException {
        ...
        if (eventDetails != null)
            visit.setVisitGroup(eventDetails.get(Constants.ICCM_VISIT_GROUP));
    ...
    public static void processHomeVisit(EventClient baseEvent, SQLiteDatabase database, String parentEventType) {
        try {
            Visit visit = MalariaLibrary.getInstance().visitRepository().getVisitByFormSubmissionID(baseEvent.getEvent().getFormSubmissionId());
            if (visit == null) {
                ...
                if (StringUtils.isNotBlank(parentEventType) && !parentEventType.equalsIgnoreCase(visit.getVisitType())) {
                    String parentVisitID = MalariaLibrary.getInstance().visitRepository().getParentVisitEventID(visit.getBaseEntityId(), parentEventType, visit.getDate());
                    visit.setParentVisitID(parentVisitID);
                }

                if (database != null) {
                    MalariaLibrary.getInstance().visitRepository().addVisit(visit, database);
                } else {
                    MalariaLibrary.getInstance().visitRepository().addVisit(visit);
                }
                    ...
                    for (Map.Entry<String, List<VisitDetail>> entry : visit.getVisitDetails().entrySet()) {
                        if (entry.getValue() != null) {
                            for (VisitDetail detail : entry.getValue()) {
                                if (database != null) {
                                    MalariaLibrary.getInstance().visitDetailsRepository().addVisitDetails(detail, database);
                                } else {
                                    MalariaLibrary.getInstance().visitDetailsRepository().addVisitDetails(detail);
                                }
                            }
                        }
                    }
